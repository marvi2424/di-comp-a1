...
%
% Figure: the report embeds "Processing Pipeline Draft.pdf" directly from
...
\title{Top-75 Discriminative Terms per Amazon Review Category\\via MapReduce on Hadoop}
\author{Group X --- Data-Intensive Computing (194.048), TU Wien\\
        \small Member 1, Member 2, Member 3, Member 4, Member 5}
\date{April 28, 2026}
...
distributed across an HDFS file of approximately 58~GB, which makes the
...
The input is a line-delimited JSON file in which every record contains at
least a \texttt{category} field and a \texttt{reviewText} field. The required
...
The full run reports $N=78\,828\,876$ reviews and 22 categories. After
preprocessing, the post-stopword vocabulary is approximately $6.2$~million
distinct words; the number of distinct $(c,t)$ pairs to be scored is
$12{,}189{,}683$.  The fact that the vocabulary grows sublinearly with
collection size (Heaps' law) is what makes a single-reducer pass over the
post-aggregated stream tractable at this scale; we return to this point in
the discussion.
...
Figure~\ref{fig:pipeline} shows the conceptual pipeline.  The implementation
realises it as four \texttt{MRStep}s linked by \texttt{mrjob}.  Phase~3
of the figure corresponds to two physical MRSteps in the code: one to
gather token totals across categories (MRStep~2) and one to compute and
rank $\chi^{2}$ scores (MRStep~3); Phase~4 of the figure corresponds to
MRStep~4 (final assembly).  All four steps run on the same cluster
job submission and share \texttt{mrjob}'s temporary HDFS directory.

\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth,keepaspectratio]{Processing Pipeline Draft.pdf}
  ...
  58~GB input. Phase~2 is \texttt{mrjob}'s internal shuffle.  Phase~3
...
\textbf{Reducer.}  A single reducer sums counts globally. Its output is
$12.19$~million $((c,w)\!\to\!\textit{count})$ records and 22
$((c,\texttt{\_\_TOTAL\_\_})\!\to\!\textit{cat\_total})$ records.
...
The reason is that the \texttt{\_\_TOTAL\_\_} sentinel must be visible to
every word reducer call, and the cleanest way to guarantee this in
\texttt{mrjob} is to funnel every key through one task; the
JSON-encoded key \texttt{"\_\_TOTAL\_\_"} sorts before any lower-case word,
so the dictionary is fully populated by the time word records arrive. The
trade-off is sequential reduce work; we discuss its actual cost in
Section~\ref{sec:results} and a parallelisable alternative in
...
hundred concurrent map containers.

\textbf{Data reduction.}  The 58~GB input is reduced to 395~MB after
...
The pipeline produces the required output and processes the full 58~GB
corpus in under 20~minutes wall-clock time.  Three design choices
...
The principal limitation is the \emph{single-reducer constraint of
MRStep~2}, which is forced by the \texttt{\_\_TOTAL\_\_} broadcast pattern.
At our data scale this cost only $\sim$3~minutes (172~s reduce time) thanks
to Heaps' law: the post-aggregation stream is bounded by the vocabulary
size, not by the raw review count.  For substantially larger corpora the
constraint would become the bottleneck, and a cleaner refactor is the
\emph{distributed-cache / side-input pattern}: a tiny preceding job
materialises the per-category totals into a small file, the main job loads
it via \texttt{--upload-file} in \texttt{mapper\_init}, and the
\texttt{\_\_TOTAL\_\_} plumbing disappears entirely. MRStep~2 then becomes
embarrassingly parallel across an arbitrary number of reducers. We did not
implement this refactor here because the current solution comfortably
meets the runtime target.

A further point worth noting is that the actual wall-clock cost of the
job is split roughly evenly between MRStep~1 (raw I/O and tokenisation
on 58~GB) and the sequential reduce phases of MRSteps~1 and~2; subsequent
steps are computationally trivial. This is consistent with the general
observation that, at this scale, MapReduce jobs are dominated by the cost
of the first heavy-aggregation pass, and that careful combiner design pays
back on every byte of input.
